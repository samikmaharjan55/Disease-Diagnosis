...
# ================================================================================================================
# IMPORTING DATA
df1 = pd.read_csv(r'C:\MIKA\Disease Diagnosis\HUNGARIAN.csv')  # DATASET CSV 1
df2 = pd.read_csv(r'C:\MIKA\Disease Diagnosis\CLEVELAND.csv')  # DATASET CSV 2
# DATASET CSV 3
df3 = pd.read_csv(r'C:\MIKA\Disease Diagnosis\SWITZERLAND.csv')
df = pd.concat([df1, df2, df3])  # MERGING 3 DIFFERENT DATASET
# SELECTING DESIRED INPUTS AND OUTPUT
dfnew = df[["age", "cp", "trestbps", "chol", "fbs", "target"]]
...
# CHOLESTEROL DATA CONVERTED INTO LIST VARIABLE
input_cp = dfnew.cp.to_numpy().astype(int).tolist()
# TARGET VALUE CONVERTED INTO LIST VARIABLE.
target = dfnew.target.to_numpy().astype(int).tolist()
...
# CHOLESTEROL DATA CONVERTED INTO LIST VARIABLE
input_chol = dfnew.chol.to_numpy().astype(int).tolist()
...
# BLOOD PRESSURE DATA CONVERTED INTO LIST VARIABLE
input_bp = dfnew.trestbps.to_numpy().astype(int).tolist()
...
# BLOOD SUGAR DATA CONVERTED INTO LIST VARIABLE
input_bsugar = dfnew.fbs.to_numpy().astype(int).tolist()
...
# AGE DATA CONVERTED INTO LIST VARIABLE
input_age = dfnew.age.to_numpy().astype(int).tolist()
...
for i in range(len(input_bp)):
    ...
    RI3 = np.minimum(ccp3, np.minimum(
        cage3, np.maximum(csug3, np.minimum(cbp3, cch3))))
    # if RI3.min() == 0:
    ...
    RI4 = np.minimum(ccp4, np.minimum(
        cage4, np.maximum(csug4, np.minimum(cbp4, cch4))))
    # if RI4.min() == 0:
    ...
    RI5 = np.minimum(ccp5, np.minimum(
        cage5, np.maximum(csug5, np.minimum(cbp5, cch5))))
    # if RI5.min() == 0:
    ...
    RI6 = np.minimum(ccp6, np.minimum(
        cage6, np.minimum(csug6, np.minimum(cbp6, cch6))))
    # if RI6.min() == 0:
    ...
    RI7 = np.minimum(ccp7, np.minimum(
        cage7, np.minimum(csug7, np.maximum(cbp7, cch7))))
    # if RI7.min() == 0:
    ...
    RI8 = np.minimum(ccp8, np.minimum(
        cage8, np.minimum(csug8, np.minimum(cbp8, cch8))))
    # if RI8.min() == 0:
    ...
    RI9 = np.minimum(ccp9, np.minimum(
        cage9, np.minimum(csug9, np.maximum(cbp9, cch9))))
    RO9 = high_risk
    ...
    RI10 = np.minimum(ccp10, np.minimum(
        cage10, np.minimum(csug10, np.maximum(cbp10, cch10))))
    RO10 = med_risk
    ...
    RI11 = np.minimum(ccp11, np.minimum(
        cage11, np.maximum(csug11, np.minimum(cbp11, cch11))))
    RO11 = high_risk
    ...
    # calculating centroid for union of rules.
    outputcp = np.trapz(R_cp * out_test, out_test) / np.trapz(R_cp, out_test)
    ...
    # calculating centroid for union of rules.
    outputchol = np.trapz(R_chol * out_test, out_test) / \
        np.trapz(R_chol, out_test)
    ...
    # calculating centroid for union of rules.
    outputbp = np.trapz(R_bp * out_test, out_test) / np.trapz(R_bp, out_test)
    ...
    # calculating centroid for union of rules.
    outputage = np.trapz(R_age * out_test, out_test) / \
        np.trapz(R_age, out_test)
    ...
    # calculating centroid for union of rules.
    outputbsugar = np.trapz(R_bsugar * out_test, out_test) / \
        np.trapz(R_bsugar, out_test)
    ...
    R_inputs = np.maximum(R_age, np.maximum(
        R_bsugar, np.maximum(R_cp, np.maximum(R_chol, R_bp))))

    # Aggregation of 8 rules + Rules for inputs
    R = np.maximum(R_inputs, np.maximum(R9, np.maximum(R8, np.maximum(R7, np.maximum(
        R6, np.maximum(R5, np.maximum(R4, np.maximum(R3, np.maximum(R2, R1)))))))))
    # print(R)
    ...
    # calculating centroid for union of rules.
    outputfinal = np.trapz(R * out_test, out_test) / np.trapz(R, out_test)
